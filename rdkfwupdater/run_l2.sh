...
# ========================================
# PKCS#11 mTLS Tests (if enabled)
# ========================================

if [ "$CI" = "true" ] && [ -n "$ENABLE_PKCS11" ]; then
    echo ""
    echo "=========================================="
    echo "[3/3] Running PKCS#11 mTLS Tests"
    echo "=========================================="
    echo ""
    
    PKCS11_RESULT_DIR="/tmp/pkcs11_test_report"
    mkdir -p "$PKCS11_RESULT_DIR"
    
    # Ensure custom OpenSSL is in PATH
    export PATH=/usr/local/bin:$PATH
    export LD_LIBRARY_PATH=/usr/local/lib64:/usr/local/lib:$LD_LIBRARY_PATH
    
    # Test: PKCS#11 Reference P12 patch with sentinel key (ID 0x2c)
    echo "[PKCS#11] Testing reference P12 with PKCS#11 patch (sentinel key → HSM)..."
    if bash test/L2-tests/pkcs11-mtls/test_pkcs11_mtls.sh > "$PKCS11_RESULT_DIR/test_pkcs11_mtls.log" 2>&1; then
        echo "✓ PKCS#11 mTLS test PASSED"
        echo "PASS" > "$PKCS11_RESULT_DIR/test_pkcs11_mtls.result"
    else
        echo "✗ PKCS#11 mTLS test FAILED"
        echo "FAIL" > "$PKCS11_RESULT_DIR/test_pkcs11_mtls.result"
    fi
    echo ""
    
    echo "PKCS#11 test results saved to: $PKCS11_RESULT_DIR/"
fi
...
if [ "$CI" = "true" ] && [ -n "$ENABLE_PKCS11" ]; then
    echo "PKCS#11 tests report: /tmp/pkcs11_test_report/"
fi
echo "=========================================="
